...
实际上，一般不建议直接修改 /etc/crontab 文件，而是在 `/etc/cron.d` 目录下创建额外的配置，该目录下的配置文件格式与 /etc/crontab 相同。但需要注意的是，此目录下的文件有命名规则，文件名由大小写字母，数字，横杠和下划线组成，**不能包括圆点**, 且文件所有者必须是 root，为软链接时，链接的文件所有者也必须是 root。
